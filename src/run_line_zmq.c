...
static int max_mpi_processor_name=1024;
...
\n\
Options:\n\
-j, --job    set num job to run\n\
-a , --add=4   input file\n\
-h, --help\n\
Example: \n\
mpirun -np 12 run_line -j 2 -a comands.txt\n\
";

static char const short_options[] = "hj:a:";
...
//MPI_Init(NULL, NULL);
    ...
    if (world_rank == 0) {
        ...
        if(namelen<num_jobs){
            printf("Warning: num_jobs %d > namelen %d, set num_jobs to namelen\n", num_jobs, namelen);
            num_jobs = namelen;
        }
